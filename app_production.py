...
# Initialize session state - FRESH START on app load (always start from 0)
if "app_initialized" not in st.session_state:
    st.session_state.app_initialized = True
    # DELETE old results file - always start from 0 on page load
    try:
        os.remove(RESULTS_FILE)
    except:
        pass

# METRICS - always show current state
try:
...
with tab_search:
    ...
    with col_left:
        ...
        if not all_venues:
            ...
            with col_date1:
                start_date = st.date_input("From", value=datetime(2026, 6, 1), key="start_date")
            with col_date2:
                end_date = st.date_input("To", value=datetime(2026, 12, 31), key="end_date")

            # City filter
            st.markdown("**Cities**")
            db = load_events_database()
            all_cities = set()
            for venue_events in db.values():
                for event in venue_events:
                    if event.get("city"):
                        all_cities.add(event.get("city"))

            selected_cities = st.multiselect(
                "Select cities (optional - leave empty for all)",
                sorted(list(all_cities)),
                default=list(sorted(all_cities)),
                key="city_multiselect"
            )

            st.markdown("---")

            search_button = st.button("START SEARCH", use_container_width=True, key="search_btn")
            ...
            if st.session_state.get("do_search", False):
                if not selected_venues:
                    ...
                    st.info(f"Loading {len(selected_venues)} venue(s)...")

                    for venue in selected_venues:
                        venue_results = search_venue_instant(venue)

                        # Filter by city only (dates are optional filter)
                        filtered_results = []
                        for event in venue_results:
                            event_city = event.get("city", "")

                            # City filter - if cities selected, only include matching cities
                            if selected_cities and event_city not in selected_cities:
                                continue
